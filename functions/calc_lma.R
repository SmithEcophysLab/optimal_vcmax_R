...
calc_lma <- function(deciduous = 'yes', # yes or no for deciduous species
  ...
  if(deciduous == 'yes'){
    ...
    # LMA <- exp(log(par_sum) - 0.052*temperature - 0.6*log(alpha) + log(f) + 1.7)
    LMA <- exp(log(f) + log(par_sum) - 0.052*temperature + Cde)
  }
  
  else{
    # evergreen LMA
    LMA <- exp(0.5*log(par_sum) - 0.013*temperature - 0.12*log(alpha) + 0.25*log(f) + 3.5)
    #LMA <- exp(0.5*log(par_sum) - 0.013*temperature + 0.25*log(f) + 3.5)
  }
